...
public class Main extends JPanel implements Runnable, MouseListener, MouseMotionListener, KeyListener {
    ...
    public static final int DYNAMITE = 10;
    public static final int EXPLOSION = 11;
    public static final int WOOD = 12;
    public static final int ICE = 13;
    public static final int OIL = 14;
    public static final int ERASER = 15;
    ...
    private void updateSmoke(int x, int y) {
        // Дым поднимается вверх (НЕ разрушает другие элементы)
        ...
        // Растекание в стороны только в пустоту
    ...
    private void updateSeed(int x, int y) {
        ...
        // Прорастание в траву ТОЛЬКО на земле и с водой рядом
        if (y < ROWS - 1) {
            int below = gridBuffer[x][y + 1];
            if (below == EARTH) {
                boolean hasWater = checkWaterNearby(x, y);
                float growthChance = hasWater ? 0.02f : 0.005f; // Быстрее с водой
    ...
    private void updateGrass(int x, int y) {
        // Рост травы ВВЕРХ только если есть земля под ней и вода рядом
        if (y > 0) {
        ...
        // Распространение травы в стороны только на землю
        if (random.nextFloat() < 0.001f) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        g.drawString("1-9,0: элементы | +/-: размер | C: очистить | ПРОБЕЛ: пауза | TAB: FPS", 10, 40);
    ...
    private Color getColorForElement(int element) {
        switch (element) {
            ...
            case ICE: return new Color(173, 216, 230, 200);
    ...
    public static void main(String[] args) {
        JFrame frame = new JFrame("PlusSandbox - Физическая Песочница+");
        Main game = new Main();
