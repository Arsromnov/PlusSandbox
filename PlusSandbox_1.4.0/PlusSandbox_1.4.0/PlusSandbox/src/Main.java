...
public class Main extends JPanel implements Runnable, MouseListener, MouseMotionListener, KeyListener {
        ...
        for (int y = ROWS - 2; y >= 0; y--) {
            for (int x = 0; x < COLS; x++) {
                ...
                switch (element) {
                    ...
                    case COAL: updateCoal(x, y); break; // НОВОЕ: уголь
                }
            }
        }
    ...
    private void updateSand(int x, int y) {
        ...
        // Песок не горит, но нагревается как металлы
        if (checkFireNearby(x, y) || checkLavaNearby(x, y)) {
            heatGrid[x][y]++;
            if (heatGrid[x][y] > 50) {
                // При сильном нагреве песок превращается в стекло
                gridBuffer[x][y] = GLASS;
                heatGrid[x][y] = 0;
            }
    ...
    private void updateCoal(int x, int y) {
        // Уголь - твердый материал, может гореть долго и выделять тепло
        if (checkFireNearby(x, y) && random.nextFloat() < 0.02f) {
            // Уголь загорается
    ...
    public void keyPressed(KeyEvent e) {
        ...
        switch (e.getKeyCode()) {
            ...
            case KeyEvent.VK_SLASH: currentElement = COAL; break; // Уголь на клавишу /
    ...
    public static void main(String[] args) {
        JFrame frame = new JFrame("PlusSandbox - Улучшенная Физическая Песочница v1.5.0");
        Main game = new Main();
